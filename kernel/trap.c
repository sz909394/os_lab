      ...
      if((sp_guard_base <= va) && (va < sp_base)){}
        ...
        if(va < p->sz){
          ...
          if(vma && (mem != 0))
          {
            ...
            if((vma->offset + offset + PGSIZE) > vma->f->ip->size)
            {
              int readn = vma->f->ip->size - (vma->offset + offset);
              if(readi(vma->f->ip, 0, (uint64)mem, (vma->offset + offset), readn) != readn )
                panic("usertrap mmap readi readn");
            }
            else{
            if(readi(vma->f->ip, 0, (uint64)mem, (vma->offset + offset), PGSIZE) != PGSIZE)
